...
export function validate(s){
 ...
 if(!s.settings||!s.upgrades||!s.standings||!s.teamStandings||!s.negotiations||!s.part2||!s.part3)throw Error('Estado incompleto.');
...
export function load(slot='auto',storage=localStorage){
 const key=PREFIX+slot,legacyKey=LEGACY_PREFIXES.map(p=>p+slot).find(k=>storage.getItem(k)),raw=storage.getItem(key)||(legacyKey&&storage.getItem(legacyKey));
 if(!raw)throw Error('Este slot está vazio.');
 try{return {state:decode(raw),recovered:false,migrated:!storage.getItem(key)&&!!legacyKey};}
 catch(error){
  const backup=storage.getItem(key+'-backup')||(legacyKey&&storage.getItem(legacyKey+'-backup'));
